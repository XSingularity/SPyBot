...
def tweetFromList(api):
	...
	with open('numero.pickle', 'rb') as fLoad:
		line_number = int(_pickle.load(fLoad))

	with open('tweetList.txt', 'r') as tweetsFile:
		f = tweetsFile.readlines()

	hashtag = input("Enter a hashtag if you want to use one for all of the tweets else just press enter: ")
	try:
		for line in f:
			line = line+" "+hashtag
			line = line.strip()
...
def sendDMToAll(api):
	...
	try:
		GetFollowers()
		messageText = input("Enter the message you want to send: ")
		for follower in followers:
